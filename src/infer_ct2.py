...
path = expanduser("~/filestore-ai/davisliang/code/llama2-13b-chat-chkpt-8600-merge_torch_float32-ct2-bfloat16")
...
pickled_data = pickle.load(open("/home/mrigankraman/filestore-ai/davisliang/data/llm/presidio10k/soap/train/train_RoS_Rest_split_no_newlines_2048.pkl","rb"))
...
for datapoint in tqdm.tqdm(data['train'].shuffle(seed=42)):
    ...
    with torch.autocast("cuda"):
        with torch.no_grad():
            ...
            print(count)
            if (count+1)%1000==0:
                with open('generations.pkl', 'wb') as file:
...
# with open('/home/mrigankraman/llama2_chat_ros_rest_split_no_newlines_8600_no_lora_bf16.csv', 'w', newline='') as csvfile:
